"""
Chargeback Workflow with Orchestrator

Demonstrates the complete 8-step chargeback workflow using the workflow orchestrator.
Shows how to chain processors and handle workflow execution.
"""
...
async def main():
    """Run orchestrated chargeback workflow"""
    
    print("="*60)
    print("Chargeback Workflow - Orchestrated Execution")
    print("="*60)
    ...
    try:
        basket = doc_ex.create_basket(
            'chargeback_orchestrated',
            description='Chargeback documents for orchestrated workflow'
        )
        print(f"✅ Created basket: {basket.id}")
    except ValueError as e:
        if 'already exists' in str(e):
            baskets = doc_ex.list_baskets()
            basket = next((b for b in baskets if b.name == 'chargeback_orchestrated'), None)
            if basket:
    ...
    temp_file = Path('temp_chargeback_orchestrated.txt')
    ...
    try:
        ...
        orchestrator.add_step('extract-identifiers', extract_processor)
        ...
        orchestrator.add_step('duplicate-check', duplicate_processor)
        ...
        contract_processor = ContractEligibilityProcessor(contract_config)
        orchestrator.add_step('contract-eligibility', contract_processor)
        ...
        gpo_processor = GpoRosterValidationProcessor(gpo_config)
        orchestrator.add_step('gpo-roster-validation', gpo_processor)
