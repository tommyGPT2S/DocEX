...
from examples.integrations._shared.prompt_manager import PromptManager, get_prompt_manager
...
class BaseLLMProcessor(BaseProcessor):
    ...
    def get_document_text(self, document: Document) -> str:
        ...
        if not content:
            raise ValueError(f"No text content available for document {document.id}")
        return content
