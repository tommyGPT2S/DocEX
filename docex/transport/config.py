...
class TransportType(str, Enum):
    """Type of DocEX transport protocol"""
    LOCAL = "local"
    SFTP = "sftp"
    HTTP = "http"
    SMTP = "smtp"
...
class RouteMethod(str, Enum):
    """Supported DocEX transport methods"""
    UPLOAD = "upload"
    DOWNLOAD = "download"
    LIST = "list"
    DELETE = "delete"
...
class OtherParty(BaseModel):
    """Represents the business entity the DocEX route connects to"""
    id: str
    name: str
...
class BaseTransportConfig(BaseModel):
    """Base configuration for DocEX transport protocols"""
    type: TransportType
    name: str
    enabled: bool = True
...
class RouteConfig(BaseModel):
    """Configuration for a DocEX transport route"""
    name: str
...
class LocalTransportConfig(BaseTransportConfig):
    """Configuration for DocEX local filesystem transport"""
    base_path: str
...
class SFTPTransportConfig(BaseTransportConfig):
    """Configuration for DocEX SFTP transport"""
    host: str
    port: int = 22
    username: str
    password: str
...
class HTTPTransportConfig(BaseTransportConfig):
    """Configuration for DocEX HTTP transport"""
    endpoint: str
    method: str = "POST"
...
class TransportConfig(BaseModel):
    """Root configuration for DocEX document transport"""
